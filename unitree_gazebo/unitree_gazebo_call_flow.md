...
Below is a PlantUML sequence diagram showing the initialization and per-cycle runtime interactions between Gazebo, the `UnitreeGazeboSystem`, ros2_control, the controller manager, controllers, and Gazebo joints/sensors. The PlantUML source is available at `docs/unitree_gazebo_sequence.puml`.

```plantuml
@startuml
title Unitree Gazebo System — Call Flow
participant Gazebo as "Gazebo / plugin loader"
participant System as "UnitreeGazeboSystem"
participant ROS2Control as "ros2_control / Resource Manager"
participant ControllerManager as "Controller Manager"
participant Controller as "Controllers"
participant GazeboJoints as "Gazebo Joints/Sensors"

Gazebo -> System: initSim(model_nh, parent_model, hardware_info, sdf)
activate System
System -> System: registerJoints(...)
System -> System: registerSensors(...)
System -> ROS2Control: export_state_interfaces()
System -> ROS2Control: export_command_interfaces()
note right of System: state & command interfaces now
deactivate System

ControllerManager -> System: perform_command_mode_switch(start, stop)
ControllerManager -> System: on_activate()

loop Control cycle
	ROS2Control -> System: read(time, period)
	System -> GazeboJoints: read joint pos/vel/effort and sensors
	Controller -> ROS2Control: update()  // compute commands
	ROS2Control -> System: command interfaces updated (position/velocity/effort/Kp/Kd)
	System -> System: update mimic joints (copy/multiply commands)
	ROS2Control -> System: write(time, period)
	System -> GazeboJoints: SetPosition / SetVelocity / SetForce
end

note over System,GazeboJoints: write() applies Kp/Kd when EFFORT mode active
